...
import { Check, KeyRound, LoaderCircle, LockKeyhole } from "lucide-react";
...
function readRecoveryUrlParams(): RecoveryUrlParams {
  ...
  const isPreparedSession = searchParams.get("recovery") === "ready";
  ...
  return {
    ...
    code,
    isPreparedSession,
    isRecovery: type === "recovery" || Boolean(code) || isPreparedSession,
  };
}
  ...
  useEffect(() => {
    ...
    const initializeRecovery = async () => {
      ...
      try {
        ...
        if (params.isPreparedSession) {
          const { data, error } = await supabase.auth.getSession();

          if (error || !data.session) {
            throw new Error("Prepared recovery session is missing.");
          }
        } else if (params.code) {
